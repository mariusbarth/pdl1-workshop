...
One of the most frequently utilized paradigms in the field of implicit learning is the serial reaction time task (SRTT) originating from Nissen and Bullemer [-@nissen_attentional_1987]. 
...
Destrebecqz and Cleeremans [-@destrebecqz_can_2001] utilized this distinction and applied the process-dissociation (PD) procedure to measure sequence learning.
...
In their application to sequence learning, Destrebecqz and Cleeremans [-@destrebecqz_can_2001] used a generation task:
...
However, performance under inclusion ($I$) and exclusion instructions was identical (i.e., $I=E$), a finding that is interpreted as indicating the absence of explicit knowledge and instead suggests that the sequence knowledge was fully implicit.^[
...
The results obtained with both tasks have been found to diverge: Using free generation task, Destrebecqz and Cleeremans [-@destrebecqz_can_2001] reported evidence for implicit knowledge (i.e., exclusion performance was above baseline). 
In contrast, Wilkinson and Shanks [-@wilkinson_intentional_2004] could not replicate this $E>B$ finding using a cued generation task. 
...
The study realized a 3 (*material*: permuted vs. random vs. no-learning) $\times$ 2 (*generation task*: free vs. cued generation) $\times$ 2 (*PD instruction*: inclusion vs. exclusion) $\times$ 2 (*block order*: inclusion first vs. exclusion first) design with repeated measures on the instruction factor. 
...
excluded_participants <- c(55, 74, 75, 126, 5, 7, 9, 12, 15)

N <- length(unique(generation$id))

n.excluded_participants <- length(unique(excluded_participants))

tmp <- aggregate(formula = correct_SOC ~ id + sex + age, data = generation, FUN=mean)
Sex <- table(tmp[["sex"]])

meanAge <- round(mean(tmp[["age"]]),digits=1)
...
The experiment consisted of three consecutive parts: First, participants worked on a SRTT (the *training phase*), followed by a *generation task* and a postexperimental interview. 
...
A horizontal sequence of six white squares (56px × 56px) was presented on a grey screen. The distance between squares was 112px. 
...
The raw data (as well as additional supplemental materials) are available from https://github.com/methexp/pdl1.^[
...
```{r srtt-1, fig.cap="Left: Mean reaction times for permuted (solid line) and random material (dashed line). Right: Mean reaction times for permuted material, split by high-frequency (filled circles) vs. low-frequency locations (open circles). Error bars represent 95% within-subjects CIs."}

# permuted vs. random
tmp <- acquisition[
  acquisition$error == 0 &
  acquisition$material != "No-learning" &
  acquisition$trial > 1 &
...
# permuted, high vs. low frequency locations
...
The mean RTs obtained over training blocks are shown in the left panel of Figure \@ref(fig:srtt-1), separately for permuted and random material (in this and all following training-phase analyses, we excluded RTs of the first trial of each block and of trials that resulted in an error). 
...
The right panel of Figure \@ref(fig:srtt-1) shows the mean RTs for these two types of stimuli. A 2 (*frequency*: high vs. low) $\times$ 6 (*block number*) ANOVA revealed a main effect of *frequency*, `r out.perm[["frequency"]]`; high-frequency responses were faster than low-frequency responses. 
The main effect of *block number* was also significant, `r out.perm[["block"]]`, but not the interaction, `r out.perm[["block_frequency"]]`.
...
We calculated the proportion of triplets that were consistent with training sequences (see Appendix A).^[
...
Figure \@ref(fig:figure-2) depicts the pattern of correctly generated triplets as a function of generation task, material, and PD instruction.
 

### Correctly generated second-order transitions

```{r figure-2, fig.cap="Proportions of correctly generated triplets during PD generation task (chance level = .2). Error bars represent 95% CIs."}
...
# separate analysis for free generation
tmp.a<-tmp2[tmp2[["generation"]]=="Free",]
out.a<-apa_print(aov_ez(data=tmp.a,id="id",dv="correct_SOC",between=c("material","order"),within=c("PD_instruction")))

# separate analysis for free generation, but without the no-learning condition
tmp.a2<-tmp2[tmp2[["generation"]] == "Free" & tmp2[["material"]] != "No-learning",]
out.a2<-apa_print(aov_ez(data=tmp.a2,id="id",dv="correct_SOC",between=c("material","order"),within=c("PD_instruction")))

# separate analysis for free generation, no-learning
tmp.ar<-tmp2[tmp2[["generation"]] == "Free" & tmp2[["material"]] == "No-learning",]
out.ar<-apa_print(aov_ez(data=tmp.ar,id="id",dv="correct_SOC",between=c("order"),within=c("PD_instruction")))


# separate analysis for free generation, exclusion
...
# separate analysis for free generation, exclusion, perm vs 0
...
# separate analysis for free generation, exclusion, perm vs rand
...
# separate analysis for free generation, exclusion, rand vs 0
...
# separate analysis for free generation, inclusion
...
# separate analysis for free generation, inclusion, perm vs 0
...
# separate analysis for free generation, inclusion, perm vs rand
...
# separate analysis for free generation, but without the perm condition
tmp.a2b<-tmp2[tmp2[["generation"]] == "Free" & tmp2[["material"]] != "Permuted",]
out.a2b<-apa_print(aov_ez(data=tmp.a2b,id="id",dv="correct_SOC",between=c("material","order"),within=c("PD_instruction")))

apa_barplot(data=tmp.a,id="id",dv="correct_SOC",ylab="Proportion correct generation",factors=c("material","PD_instruction"),ylim=c(0,1), main="Free", intercept=.2, font.main=1)

# only cued generation
tmp.b<-tmp2[tmp2[["generation"]]=="Cued",]
out.b<-apa_print(aov_ez(data=tmp.b,id="id",dv="correct_SOC",between=c("material","order"),within=c("PD_instruction")))

apa_barplot(data=tmp.b,id="id",dv="correct_SOC",ylab="Proportion correct generation",factors=c("material","PD_instruction"), ylim=c(0,1), main="Cued", intercept=.2)



# Post-hoc paired comparisons for free generation group
fit <- aov_ez(data=tmp.a,id="id",dv="correct_SOC",between=c("material","order"),within=c("PD_instruction"))
...
We first applied the ordinal PD approach [@hirshman_ordinal_2004]. 
It proposes a model-free analysis strategy of PD data by deriving four critical data patterns of inclusion and exclusion performance changes across two conditions that imply differences in automatic and controlled processes between the conditions.
For instance, it can be concluded that both the automatic and the controlled processes are stronger in one condition if inclusion performance is greater in that condition but exclusion performance is comparable in both conditions.
Also, it can be concluded that the automatic process is greater in one condition if both inclusion and exclusion performance are greater in that condition.

We screened the free-generation data for these data patterns by separately comparing inclusion and exclusion performance across conditions of the material factor.
Inclusion performance was affected by material, `r out.ai$full_result[["material"]]`: it was greater in the *permuted* than in both the *no-learning* group, `r out.aip0$full_result[["material"]]`, and the *random* group, `r out.aipr$full_result[["material"]]`; in the latter two, performance was comparable, `r out.air0$full_result[["material"]]`.
Exclusion performance was also affected by material, `r out.ae$full_result[["material"]]`; specifically, it was greater in the *permuted* than the *random* condition, `r out.aepr$full_result[["material"]]`, but did not differ between the other two conditions (*no-learning* vs. *permuted*: `r out.aep0$full_result[["material"]]`; *no-learning* vs. *random*: `r out.aer0$full_result[["material"]]`).
...
```{r figure-6, fig.cap="Proportions of locations generated in the cued generation task that were presented as a cue on their respective trial, separately for the first and the second generation block (chance level = .6). Error bars represent 95% CIs."}
...
For the cued generation task, Figure \@ref(fig:figure-6) shows the proportion of response locations that were also presented as a cue on their respective trial.
...
This effect was qualified by an interaction *PD instruction* $\times$ *block order*, `r out$full_result[["order_instruction"]]`, which indicates that the effect of PD instruction differed between participants who worked under inclusion instructions in the first generation block and under exclusion instructions in the second block and those who received the PD instructions in the reverse order. To further explore this interaction, we analysed first and second blocks using separate ANOVAs, thus turning the *PD instruction* factor into a between-subjects factor. 
...
This data pattern remained even after excluding reversals: Analysing only the first block, only the main effect of *material* trended to be significant, `r out_1st[["material"]]`. Analysing only the second block revealed a main effect of *PD instruction*, `r out_2nd[["PD_instruction"]]`.
]

```{r fig.cap = "Proportions of locations generated in the cued generation task that were presented as cues on this trial, excluding reversals. Error bars represent 95% CIs."}
# Reversals excluded ----
tmp <- generation[
  generation$included_participant &
  generation[["trial"]] > 1 &
...
Under those circumstances, the ordinal PD approach is no longer valid [@hirshman_ordinal_2004].^[
...
Because baseline performance may be different under inclusion and exclusion instructions, two different parameters $R_{inclusion}$ and $R_{exclusion}$ are added to the model. 
The equations of the experimental conditions of the extended model are: 
$p(correct|inclusion, experimental) = C + (1-C)*A + (1-C)(1-A)*R_{inclusion}$,
and
$p(correct|exclusion, experimental) = (1-C)*A + (1-C)(1-A)*R_{exclusion}$.
...
$p(correct|inclusion, control) = R_{inclusion}$,
and
$p(correct|exclusion, control) = R_{exclusion}$.
...
Estimates of $R_{inclusion}$ and $R_{exclusion}$ therefore reflect generation performance in the random group, $R_{inclusion} = .251$ and $R_{exclusion} = .228$.
...
In other words, the extended model requires additional assumptions, namely that the processes underlying the parameters $C$ and $A$ are independent from the nuisance processes (e.g., guessing, response bias) that are reflected in the new parameters $R_{inclusion}$ and $R_{exclusion}$.
...
As mentioned above, the extended PD model assumes that response tendencies as modelled by $R_{inclusion}$ and $R_{exclusion}$ are independent of controlled and automatic processes. 
...
As a consequence of aggregation in the presence of heterogeneity, confidence intervals can be underestimated, and nominal statistical error levels can be violated by statistical tests.^[
...
In all cases in which parameters must be assumed to vary across participants, they should be preferred to the traditional approach of data aggregation.^[
